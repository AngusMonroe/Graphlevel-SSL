...
class TUDataset_aug(InMemoryDataset):
    ...
    def get(self, idx):
        ...
        if self.aug == 'dnodes':
            ...
            data_aug.x = torch.ones((data.edge_index.max()+1, 1)) # augmentation을 none으로 줘도 feature를 1로 채움
            ...
            data_aug = compute_ppr(deepcopy(data))
...
class TUDataset_aug2(InMemoryDataset):
    r"""A variety of graph kernel benchmark datasets, *.e.g.* "IMDB-BINARY",
    "REDDIT-BINARY" or "PROTEINS", collected from the `TU Dortmund University
    <https://chrsmrrs.github.io/datasets>`_.
    In addition, this dataset wrapper provides `cleaned dataset versions
    ...
    def __init__(self, root, name, transform=None, pre_transform=None,
                 pre_filter=None, use_node_attr=False, use_edge_attr=False,
                 cleaned=False, aug=None, aug2=None):
        self.name = name
        self.cleaned = cleaned
        super(TUDataset_aug2, self).__init__(root, transform, pre_transform,
                                        pre_filter)
        self.data, self.slices = torch.load(self.processed_paths[0])
        if self.data.x is not None and not use_node_attr:
            num_node_attributes = self.num_node_attributes
    ...
    def get(self, idx):
        ...
        if self.aug2 == 'dnodes':
            data_aug2 = drop_nodes(deepcopy(data))
        elif self.aug2 == 'pedges':
            data_aug2 = permute_edges(deepcopy(data))
        elif self.aug2 == 'subgraph':
            data_aug2 = subgraph(deepcopy(data))
        elif self.aug2 == 'mask_nodes':
            data_aug2 = mask_nodes(deepcopy(data))
        elif self.aug2 == 'none':
            """
            if data.edge_index.max() > data.x.size()[0]:
                print(data.edge_index)
            ...
            data_aug2.x = torch.ones((data.edge_index.max()+1, 1)) # augmentation을 none으로 줘도 feature를 1로 채움


        return data, data_aug, data_aug2
...
def compute_ppr(data, alpha=0.2, self_loop=True):
    ...
    u = data.edge_index
    #v = torch.ones_like(u.)
    #v = torch.ones(data_aug.edge_index.size(1))
    v = torch.ones_like(u[0])
    size = (torch.max(data.edge_index)+1).item()
    a = torch.sparse_coo_tensor(u, v, (size,size)).to_dense()
    a = np.array(a)

    if self_loop:
        a = a + np.eye(a.shape[0]) # self loop
    d = np.diag(np.sum(a, 1))                                     # D^ = Sigma A^_ii
    dinv = fractional_matrix_power(d, -0.5)                       # D^(-1/2)
    at = np.matmul(np.matmul(dinv, a), dinv)                      # A~ = D^(-1/2) x A^ x D^(-1/2)

    adj = alpha * inv((np.eye(a.shape[0]) - (1 - alpha) * at))   # a(I_n-(1-a)A~)^-1
