...
class GraphCL_Trainer():
    ...
    def _init(self):
        ...
        self._dataset = TUDataset(path, name=args.name, aug=args.aug).shuffle()
        ...
        self._model = GraphCL(dataset_num_features, args).to(self._device)
        print(self._model)
    ...
    def experiment(self):
        ...
        for epoch in range(1, self._args.epochs+1):
            ...
            for data in self._loader:
                ...
                x = self._model(data.x, data.edge_index, data.batch, data.num_graphs)
                ...
                x_aug = self._model(data_aug.x, data_aug.edge_index, data_aug.batch, data_aug.num_graphs)

                loss = self._model.loss_cal(x, x_aug)
